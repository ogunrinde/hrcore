...
$appraisal = [];
  ...
  $query = "SELECT * FROM appraisal INNER JOIN appraisal_replies ON appraisal_replies.appraisal_id = appraisal.id INNER JOIN users ON users.id = appraisal_replies.staff_id WHERE appraisal_replies.admin_id = '".$_SESSION['user']['id']."'";
                  ...
                  <div class="x_panel">
                    ...
                    <div class="x_content">
                      ...
                      <ul class="list-group">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center allappraisal" id = "getdata<?=$i?>" appraisal_id = "<?=$appraisal[$i]['appraisal_id']?>" staff_id = "<?=$appraisal[$i]['staff_id']?>" style ="cursor: pointer;">
                            <span><i class="fa fa-file" style="margin-right: 7px;"></i><?= $appraisal[$i]['period'] ?> (<?= $appraisal[$i]['year'] ?>)</span>
                            <span class="badge badge-primary badge-pill" style="text-transform: uppercase;font-size: 15px;border-radius: 2px;background-color: #1ABB9C"><?=$appraisal[$i]['name']?>  <?=$appraisal[$i]['fname']?></span>
                        </li>
                        <?php }else { ?>
                        <li class="list-group-item d-flex justify-content-between align-items-center allappraisal" id = "getdata<?=$i?>" appraisal_id = "<?=$appraisal[$i]['appraisal_id']?>" staff_id = "<?=$appraisal[$i]['staff_id']?>" style ="cursor: pointer;">
